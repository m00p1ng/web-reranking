...
public class IndexFilesWithMoreInfo {
    ...
    public static void main(String[] args) {
        ...
        if (docsPath == null || pageRankPath == null || urlmapPath == null) {
            System.err.println("Usage: " + usage);
            System.exit(1);
        }

        docDir = Paths.get(docsPath);
        Path prPath = Paths.get(pageRankPath);
        Path umPath = Paths.get(urlmapPath);
        if (!Files.isReadable(docDir) || !Files.isReadable(prPath) || !Files.isReadable(umPath)) {
            System.out.println("Document directory '" + docDir.toAbsolutePath() + "' does not exist or is not readable, please check the path");
            System.exit(1);
        }

        try {
            readPageRankFile(prPath, umPath);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    static void readPageRankFile(Path prPath, Path urlmapPath) throws IOException {
        List<Double> pageScore = new ArrayList<>();
        List<String> urlmapList = new ArrayList<>();

        try (InputStream stream = Files.newInputStream(prPath)) {
            BufferedReader br = new BufferedReader(new InputStreamReader(stream));
            String line = br.readLine();

            while (line != null) {
                pageScore.add(Double.parseDouble(line.trim()));
                line = br.readLine();
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        try (InputStream stream = Files.newInputStream(urlmapPath)) {
            BufferedReader br = new BufferedReader(new InputStreamReader(stream));
            String line = br.readLine();

            while (line != null) {
                urlmapList.add(line.trim());
                line = br.readLine();
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        for (int i = 0; i < pageScore.size(); i++) {
            PageRank.put(urlmapList.get(i), pageScore.get(i));
        }
    }
}